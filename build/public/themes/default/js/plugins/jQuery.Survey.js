...
(function ($) {
  ...
  $.fn[PLUGIN_NS].defaults = {
    ...
    templates: {
      ...
      radio: '<div class="input" {{compareHook element}}><div class="inputLabel" name="{{element.value}}">{{element.label}}{{#if element.required}}<span class="ast">*</span>{{/if}}<span class="colon">:</span></div>'+
             '{{#each element.options}}'+
               '<label><input {{#if ../element.required}} data-validation="required"{{/if}} type="radio" name="{{../element.name}}" {{onChangeUpdateById ../element.id "_" key}} value="{{key}}"{{#dataExists ../element.name key}} checked{{/dataExists}} /><label ></label><span>{{value}}</span></label>'+
             '{{/each}}</div>',
      checkbox: '{{#if element.options}}'+
                  ...
                  '{{#each element.options}}'+
                    '<label><input type="checkbox" name="{{../element.name}}[]" {{onChangeUpdateById ../element.id "_" key}} value="{{key}}" {{#dataExists ../element.name key}}checked {{/dataExists}}{{#if ../element.required}} required{{/if}} /><label ></label><span>{{value}}</span></label>'+
                  '{{/each}}</div>'+
                '{{else}}'+
