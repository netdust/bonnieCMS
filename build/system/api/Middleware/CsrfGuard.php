 ...
 *
 * $api = new \Slim\Slim();
 * $api->add(new \Slim\Extras\Middleware\CsrfGuard());
 *
 */
...
class CsrfGuard extends \Slim\Middleware
{
    ...
    public function check() {
        ...
        if (in_array($this->app->request()->getMethod(), array('POST', 'PUT', 'DELETE'))) {
            $normalized_header = 'HTTP_'.str_replace('-', '_', strtoupper($this->header));
            $userToken = $this->app->request()->post($this->key) ?: $this->app->environment()[$normalized_header];
            if ($token !== $userToken) {
                $this->app->halt(400, 'Invalid or missing CSRF token. ' . print_r( $this->app->environment(), true ));
            }
        }
